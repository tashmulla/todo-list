...
def update_todo_text(todo_id: int, todo: schemas.TodoUpdateText, db: Session = Depends(database.get_db)):
    return crud.update_todo_text(db, todo_id, todo.text)

@router.patch("/todos/{todo_id}/completed", response_model=schemas.Todo)
def update_todo_completed(todo_id: int, todo: schemas.TodoUpdateCompleted, db: Session = Depends(database.get_db)):
    return crud.update_todo_completed(db, todo_id, todo.completed)

@router.delete("/todos/completed", response_model=int)
def delete_completed_todos(db: Session = Depends(database.get_db)):
